...
<template>
	<div v-if="show" class="fixed inset-0 z-10 flex items-center justify-center bg-black bg-opacity-40">
		<div
			class="bg-industrial-panel bg-black bg-opacity-70 rounded-xl shadow-industrial p-6 w-[90vw] max-w-lg mx-auto text-center"
		>
			...
			<div class="flex flex-col gap-3">
				<button
					class="industrial-button w-full text-white font-bold py-3 px-4 text-sm uppercase tracking-wide"
					@click="onAccept"
				>
					{{ t("restorePrompt.continueButton") }}
				</button>
				<button
					class="w-full text-white font-bold py-3 px-4 text-sm uppercase tracking-wide bg-industrial-red hover:bg-red-700"
					@click="onReject"
				>
